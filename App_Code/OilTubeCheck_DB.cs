...
/// <summary>
/// OilTubeCheck_DB 的摘要描述
/// </summary>
public class OilTubeCheck_DB
{
    ...
    public DataTable GetList()
    {
        ...
        sb.Append(@"select * from 石油_管線巡檢 where 資料狀態='A' and 業者guid=@業者guid ");
    ...
    public DataTable GetList2()
    {
        ...
        sb.Append(@"select * from 石油_管線巡檢_異常情形統計資料 where 資料狀態='A' and 業者guid=@業者guid ");
        if (!string.IsNullOrEmpty(年度))
            sb.Append(@" and 年度=@年度");
...
select DISTINCT 年度 into #tmp from 石油_管線巡檢
...
if(@yearCount > 0)
    ...
    public DataTable GetData()
    {
        ...
        sb.Append(@"select * from 石油_管線巡檢_異常情形統計資料 where guid=@guid and 資料狀態='A' ");
...
where guid=@guid and 資料狀態=@資料狀態 
 ");
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"insert into 石油_管線巡檢_異常情形統計資料(  
年度,
業者guid,
管線巡檢情形,
前兩年,
...
@資料狀態  
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"update 石油_管線巡檢_異常情形統計資料 set  
年度=@年度, 
管線巡檢情形=@管線巡檢情形, 
前兩年=@前兩年, 
前一年=@前一年, 
...
where guid=@guid and 資料狀態=@資料狀態 
 ");
    ...
    public void DeleteData()
    {
        SqlCommand oCmd = new SqlCommand();
        oCmd.Connection = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString"]);
        oCmd.CommandText = @"update 石油_管線巡檢_異常情形統計資料 set 
修改日期=@修改日期, 
修改者=@修改者, 
資料狀態='D' 
where guid=@guid ";
