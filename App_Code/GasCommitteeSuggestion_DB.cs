...
from 天然氣_自評表委員意見log where 題目guid=@題目guid and 業者guid=@業者guid and 資料狀態='A' 
and 年度=@年度) R 
where R.SN=1) 

select * from 天然氣_自評表委員意見log 
where guid=@logguid
");
        SqlCommand oCmd = oConn.CreateCommand();
        oCmd.CommandText = sb.ToString();
...
select @count=count(*) from 天然氣_自評表委員意見log where 題目guid=@題目guid and 資料狀態=@資料狀態 
and 業者guid=@業者guid and 年度=@年度

SET @sort =(select R.SN from(
select *,
Row_Number() over (PARTITION BY 題目guid order by 修改日期 desc) SN
from 天然氣_自評表委員意見log where 題目guid=@題目guid and 資料狀態=@資料狀態 and 年度=@年度) R
where  R.guid=@guid) 
