...
public class OilCIPS_DB
{
    ...
    string 排成改善_數量 = string.Empty;
    string 排成改善_改善完成數量 = string.Empty;
    string 需監控點數量 = string.Empty;
    ...
    public string _排成改善_數量 { set { 排成改善_數量 = value; } }
    public string _排成改善_改善完成數量 { set { 排成改善_改善完成數量 = value; } }
    public string _需監控點數量 { set { 需監控點數量 = value; } }
    ...
    public DataTable GetList()
    {
        ...
        sb.Append(@"select * from 石油_緊密電位檢測表 where 資料狀態='A' and 業者guid=@業者guid ");
...
select DISTINCT 年度 into #tmp from 石油_緊密電位檢測表
...
if(@yearCount > 0)
    ...
    public DataTable GetData()
    {
        ...
        sb.Append(@"select * from 石油_緊密電位檢測表 where guid=@guid and 資料狀態='A' ");
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"insert into 石油_緊密電位檢測表(  
年度,
業者guid,
長途管線識別碼,
同時檢測管線數量,
...
@資料狀態  
        ...
        oCmd.Parameters.AddWithValue("@排成改善_數量", 排成改善_數量);
        oCmd.Parameters.AddWithValue("@排成改善_改善完成數量", 排成改善_改善完成數量);
        oCmd.Parameters.AddWithValue("@需監控點數量", 需監控點數量);
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"update 石油_緊密電位檢測表 set  
長途管線識別碼=@長途管線識別碼,
同時檢測管線數量=@同時檢測管線數量,
最近一次執行年月=@最近一次執行年月,
報告產出年月=@報告產出年月,
...
where guid=@guid and 資料狀態=@資料狀態 
 ");
    ...
    public void DeleteData()
    {
        SqlCommand oCmd = new SqlCommand();
        oCmd.Connection = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString"]);
        oCmd.CommandText = @"update 石油_緊密電位檢測表 set 
修改日期=@修改日期, 
修改者=@修改者, 
資料狀態='D' 
where guid=@guid ";
