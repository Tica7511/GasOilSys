...
/// <summary>
/// OilRiskAssessment_DB 的摘要描述
/// </summary>
public class OilRiskAssessment_DB
{
    ...
    public string _改善後風險等級 { set { 改善後風險等級 = value; } }
    ...
    public DataTable GetList()
    {
        ...
        sb.Append(@"select * from 石油_風險評估資料 where 資料狀態='A' and 業者guid=@業者guid ");
...
select DISTINCT 年度 into #tmp from 石油_風險評估資料
...
if(@yearCount > 0)
    ...
    public DataTable GetData()
    {
        ...
        sb.Append(@"select * from 石油_風險評估資料 where guid=@guid and 資料狀態='A' ");
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"insert into 石油_風險評估資料(  
年度,
業者guid,
長途管線識別碼,
最近一次執行日期,
...
@資料狀態  
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"update 石油_風險評估資料 set  
年度=@年度,
長途管線識別碼=@長途管線識別碼,
最近一次執行日期=@最近一次執行日期,
再評估時機=@再評估時機,
...
where guid=@guid and 資料狀態=@資料狀態 
 ");
    ...
    public void DeleteData()
    {
        SqlCommand oCmd = new SqlCommand();
        oCmd.Connection = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString"]);
        oCmd.CommandText = @"update 石油_風險評估資料 set 
修改日期=@修改日期, 
修改者=@修改者, 
資料狀態='D' 
where guid=@guid ";
