...
public partial class Handler_AddGasRiskAssessment : System.Web.UI.Page
{
    GasRiskAssessment_DB gdb = new GasRiskAssessment_DB();
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            gdb._最近一次執行日期 = Server.UrlDecode(txt2_1) + "/" + Server.UrlDecode(txt2_2);
            gdb._再評估時機 = Server.UrlDecode(txt3);
            gdb._管線長度 = Server.UrlDecode(txt4);
            gdb._分段數量 = Server.UrlDecode(txt5);
            gdb._已納入ILI結果 = Server.UrlDecode(txt6);
            gdb._已納入CIPS結果 = Server.UrlDecode(txt7);
            gdb._已納入巡管結果 = Server.UrlDecode(txt8);
            gdb._各等級風險管段數量_高 = Server.UrlDecode(txt9);
            gdb._各等級風險管段數量_中 = Server.UrlDecode(txt10);
            gdb._各等級風險管段數量_低 = Server.UrlDecode(txt11);
            gdb._降低中高風險管段之相關作為文件名稱 = Server.UrlDecode(txt12);
            gdb._改善後風險等級高中低 = Server.UrlDecode(txt13);
