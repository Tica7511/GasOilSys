		...
		try
		{
			...
			if (qdt.Rows.Count > 0)
			{
				for (int i = 0; i < qdt.Rows.Count; i++)
				{
					...
					string vfStr = (string.IsNullOrEmpty(Request["fv_" + qdt.Rows[i]["天然氣自評表題目guid"].ToString()])) ? "" : Request["fv_" + qdt.Rows[i]["天然氣自評表題目guid"].ToString()].ToString().Trim();
            ...
            if (adt.Rows.Count > 0)
            {
                ...
                {
                    string cAns = (string.IsNullOrEmpty(Request["cg_" + adt.Rows[i]["天然氣自評表分類guid"].ToString()])) ? "" : Request["cg_" + adt.Rows[i]["天然氣自評表分類guid"].ToString()].ToString().Trim();
                    string mAns = (string.IsNullOrEmpty(Request["mg_" + adt.Rows[i]["天然氣自評表分類guid"].ToString()])) ? "" : Request["mg_" + adt.Rows[i]["天然氣自評表分類guid"].ToString()].ToString().Trim();
                    string PsStr = (string.IsNullOrEmpty(Request["ps_" + adt.Rows[i]["天然氣自評表分類guid"].ToString()])) ? "" : Request["ps_" + adt.Rows[i]["天然氣自評表分類guid"].ToString()].ToString().Trim();
                    string vfStr = (string.IsNullOrEmpty(Request["fv_" + adt.Rows[i]["天然氣自評表分類guid"].ToString()])) ? "" : Request["fv_" + adt.Rows[i]["天然氣自評表分類guid"].ToString()].ToString().Trim();
                    ...
                    if (LogInfo.competence == "02") // 業者
                    {
                        if (cAns != "")
                        {
                            ...
                            ans_db._題目guid = adt.Rows[i]["天然氣自評表分類guid"].ToString();
                            ans_db._年度 = adt.Rows[i]["天然氣自評表分類年份"].ToString();
                            ans_db._填寫人員類別 = LogInfo.competence;
                            ans_db._建立者 = LogInfo.mGuid;
                            ans_db._修改者 = LogInfo.mGuid;
                            #endregion
                    ...
                    else //管理者
                    {
                        ...
                        if (mAns != "")
                        {
                            ...
                            cs_db._委員 = LogInfo.name;
                            cs_db._業者guid = LogInfo.companyGuid;
                            cs_db._題目guid = adt.Rows[i]["天然氣自評表分類guid"].ToString();
                            cs_db._年度 = adt.Rows[i]["天然氣自評表分類年份"].ToString();
                            cs_db._答案 = mAns;
                            cs_db._檢視文件 = vfStr;
                            cs_db._委員意見 = pStr;
                            cs_db._建立者 = LogInfo.mGuid;
