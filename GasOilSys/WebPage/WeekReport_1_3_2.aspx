<%@ Page Title="" Language="C#" MasterPageFile="~/WebPage/MasterPageWeek.master" AutoEventWireup="true" CodeFile="WeekReport_1_3_2.aspx.cs" Inherits="WebPage_WeekReport_1_3_2" %>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $(document).on("click", "a[name='editbtn']", function () {
                ...
                $("#txt_tank").val($("span[name='ta_" + $("#HGuid").val() + "']").html());                
                $("#txt_pipeline").val($("span[name='pl_" + $("#HGuid").val() + "']").html());                
                $("#txt_disaster").val($("span[name='di_" + $("#HGuid").val() + "']").html());                
                $("#txt_energy").val($("span[name='en_" + $("#HGuid").val() + "']").html());                
                $("#txt_oil").val($("span[name='oil_" + $("#HGuid").val() + "']").html());
            ...
            $(document).on("click", "#subbtn", function () {
                ...
                $.ajax({
			    	...
			    	url: "../Handler/AddWeekReport_1_3_2.aspx",
			    	data: {
			    		mode: mode,
			    		rid: $("#HGuid").val(),
                        ...
                        unit: $("#txt_unit").val(),
                        tank: $("#txt_tank").val(),
                        pipeline: $("#txt_pipeline").val(),
                        disaster: $("#txt_disaster").val(),
                        energy: $("#txt_energy").val(),
                        oil: $("#txt_oil").val(),
			    		time: $("#txt_time").val(),
			    		other: $("#txt_other").val(),
			    	},
			    	...
			    	success: function (data) {
			    		if ($(data).find("Error").length > 0) {
                            ...
                            location.href = "WeekReport_1_3_2.aspx?rid=" + $("#RGuid").val() + "&rpid=" + $("#RPGuid").val() + "&year=" + $.getQueryString("year");
			    		}
			    	}
			    });
            });
            ...
            $(document).on("click", "a[name='delbtn']", function () {
                ...
                if (str) {
                    $.ajax({
			        	type: "POST",
			        	async: false, //在沒有返回值之前,不會執行下一步動作
			        	url: "../Handler/DelWeekReport_1_3_2.aspx",
			        	data: {
                            rid: $(this).attr("aid"),
        ...
        function getData() {
			$.ajax({
				type: "POST",
				async: false, //在沒有返回值之前,不會執行下一步動作
				url: "../Handler/GetWeekReport_1_3_2.aspx",
				data: {
					...
					if ($(data).find("Error").length > 0) {
						...
						if ($(data).find("data_item2").length > 0) {
                            $(data).find("data_item2").each(function (i) {                                
                                ...
                                tabstr += '</td>';
                                tabstr += '<td>';
                                tabstr += '<span name="pl_' + $(this).children("guid").text().trim() + '">' + $(this).children("管線").text().trim() + '</span>'
                                tabstr += '</td>';
                                tabstr += '<td>';
                                tabstr += '<span name="ta_' + $(this).children("guid").text().trim() + '">' + $(this).children("儲槽").text().trim() + '</span>'
                                tabstr += '</td>';                                
                                tabstr += '<td>';
                                tabstr += '<span name="di_' + $(this).children("guid").text().trim() + '">' + $(this).children("災害救防").text().trim() + '</span>'
                                tabstr += '</td>';
                                tabstr += '<td>';
                                tabstr += '<span name="en_' + $(this).children("guid").text().trim() + '">' + $(this).children("能源局").text().trim() + '</span>'
                                tabstr += '</td>';
                                tabstr += '<td>';
                                tabstr += '<span name="oil_' + $(this).children("guid").text().trim() + '">' + $(this).children("中油儲運處").text().trim() + '</span>'
                                tabstr += '</td>';                                
                                tabstr += '<td>';
            ...
            <div class="OchiTrasTable width100 TitleLength08 font-size3">
                <div class="OchiRow">
                    ...
                    <div class="OchiCell width100">
                        <input id="txt_unit" type="text" class="inputex width100" />
                    </div>
                </div><!-- OchiRow -->
                <div class="OchiRow">
                    <div class="OchiHalf">
                        ...
                        <div class="OchiCell width100">
                            <input id="txt_time" type="text" class="inputex pickDate width85" disabled>
                        </div>
                    </div><!-- OchiHalf -->
                    <div class="OchiHalf">
                        <div class="OchiCell OchiTitle IconCe TitleSetWidth">管線</div>
                        <div class="OchiCell width100">
                            <input id="txt_pipeline" type="text" class="inputex width100" />
                        </div>
                    </div><!-- OchiHalf -->                                       
                </div><!-- OchiRow -->                
                <div class="OchiRow">
                    <div class="OchiHalf">
                        ...
                        <div class="OchiCell width100">
                            <input id="txt_tank" type="text" class="inputex width100" />
                        </div>
                    </div><!-- OchiHalf --> 
                    <div class="OchiHalf">
                        <div class="OchiCell OchiTitle IconCe TitleSetWidth">災害救防</div>
                        <div class="OchiCell width100">
                            <input id="txt_disaster" type="text" class="inputex width100" />
                        </div>
                    </div><!-- OchiHalf -->                    
                </div><!-- OchiRow -->                 
                <div class="OchiRow">
                    ...
                    <div class="OchiHalf">
                        <div class="OchiCell OchiTitle IconCe TitleSetWidth">中油儲運處</div>
                        <div class="OchiCell width100">
                            <input id="txt_oil" type="text" class="inputex width100" />
                        </div>
                    </div>                    
                </div><!-- OchiRow -->                
                <div class="OchiRow">
