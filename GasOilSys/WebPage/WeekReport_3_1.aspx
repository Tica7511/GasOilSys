<%@ Page Title="" Language="C#" MasterPageFile="~/WebPage/MasterPageWeek.master" AutoEventWireup="true" CodeFile="WeekReport_3_1.aspx.cs" Inherits="WebPage_WeekReport_3_1" %>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $(document).on("click", "a[name='editbtn']", function () {
                ...
                $("#txt_place").val($("span[name='pc_" + $("#HGuid").val() + "']").html());
            ...
            $(document).on("click", "#subbtn", function () {
                ...
                $.ajax({
			    	...
			    	url: "../Handler/AddWeekReport_3_1.aspx",
			    	data: {
			    		mode: mode,
			    		rid: $("#HGuid").val(),
			    	...
			    	success: function (data) {
			    		if ($(data).find("Error").length > 0) {
                            ...
                            location.href = "WeekReport_3_1.aspx?rid=" + $("#RGuid").val() + "&rpid=" + $("#RPGuid").val() + "&year=" + $.getQueryString("year");
			    		}
			    	}
			    });
            });
            ...
            $(document).on("click", "a[name='delbtn']", function () {
                ...
                if (str) {
                    $.ajax({
			        	type: "POST",
			        	async: false, //在沒有返回值之前,不會執行下一步動作
			        	url: "../Handler/DelWeekReport_3_1.aspx",
			        	data: {
			        		rid: $(this).attr("aid"),
			        	},
			        	error: function (xhr) {
        ...
        function getData() {
			$.ajax({
				type: "POST",
				async: false, //在沒有返回值之前,不會執行下一步動作
				url: "../Handler/GetWeekReport_3_1.aspx",
				data: {
					...
					if ($(data).find("Error").length > 0) {
						...
						if ($(data).find("data_item2").length > 0) {
                            $(data).find("data_item2").each(function (i) {                                
                                ...
                                tabstr += '</td>';
                                tabstr += '<td>';
                                tabstr += '<span name="pc_' + $(this).children("guid").text().trim() + '">' + $(this).children("地點").text().trim() + '</span>'
                                tabstr += '</td>';
                                tabstr += '<td td align="center">';
                    ...
                    <table id="tablist" width="100%" border="0" cellspacing="0" cellpadding="0">
                        <thead>
                            <tr>
                                ...
                                <th nowrap="nowrap" width="100">地點</th>
