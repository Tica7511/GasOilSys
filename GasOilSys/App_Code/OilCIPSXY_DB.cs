...
public class OilCIPSXY_DB
{
    ...
    public DataTable GetList()
    {
        ...
        sb.Append(@"select * from 石油_緊密電位檢測_xy座標 where 資料狀態='A' and 父層guid=@父層guid ");
...
select DISTINCT 年度 into #tmp from 石油_緊密電位檢測_xy座標
...
if(@yearCount > 0)
    ...
    public DataTable GetData()
    {
        ...
        sb.Append(@"select * from 石油_緊密電位檢測_xy座標 where guid=@guid and 資料狀態='A' ");
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"insert into 石油_緊密電位檢測_xy座標(  
父層guid,
年度,
業者guid,
x座標,
...
@資料狀態  
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"update 石油_緊密電位檢測_xy座標 set  
x座標=@x座標,
y座標=@y座標,
級距=@級距,
修改者=@修改者, 
...
where guid=@guid and 資料狀態=@資料狀態 
 ");
    ...
    public void DeleteData()
    {
        SqlCommand oCmd = new SqlCommand();
        oCmd.Connection = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString"]);
        oCmd.CommandText = @"update 石油_緊密電位檢測_xy座標 set 
修改日期=@修改日期, 
修改者=@修改者, 
資料狀態='D' 
where guid=@guid ";
