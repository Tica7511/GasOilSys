...
/// <summary>
/// OilTubeMaintain_DB 的摘要描述
/// </summary>
public class OilTubeMaintain_DB
{
    ...
    string 前一年度管線變更性質 = string.Empty;
    string 長度 = string.Empty;
    string 管段位置 = string.Empty;
    ...
    public string _前一年度管線變更性質 { set { 前一年度管線變更性質 = value; } }
    public string _長度 { set { 長度 = value; } }
    public string _管段位置 { set { 管段位置 = value; } }
    ...
    public DataTable GetList()
    {
        ...
        sb.Append(@"select * from 石油_管線維修或開挖 where 資料狀態='A' and 業者guid=@業者guid ");
...
select DISTINCT 年度 into #tmp from 石油_管線維修或開挖
...
if(@yearCount > 0)
    ...
    public DataTable GetData()
    {
        ...
        sb.Append(@"select * from 石油_管線維修或開挖 where guid=@guid and 資料狀態='A' ");
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"insert into 石油_管線維修或開挖(  
年度,
業者guid,
長途管線識別碼,
前一年度管線變更性質,
...
@資料狀態  
        ...
        oCmd.Parameters.AddWithValue("@前一年度管線變更性質", 前一年度管線變更性質);
        oCmd.Parameters.AddWithValue("@長度", 長度);
        oCmd.Parameters.AddWithValue("@管段位置", 管段位置);
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"update 石油_管線維修或開挖 set  
年度=@年度,
長途管線識別碼=@長途管線識別碼,
前一年度管線變更性質=@前一年度管線變更性質,
長度=@長度,
管段位置=@管段位置,
備註=@備註,
修改者=@修改者, 
...
where guid=@guid and 資料狀態=@資料狀態 
 ");
    ...
    public void DeleteData()
    {
        SqlCommand oCmd = new SqlCommand();
        oCmd.Connection = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString"]);
        oCmd.CommandText = @"update 石油_管線維修或開挖 set 
修改日期=@修改日期, 
修改者=@修改者, 
資料狀態='D' 
where guid=@guid ";
