...
public class GasControl_DB
{
    ...
    string 負責泵送或接收之控制室名稱 = string.Empty;
    string 操作壓力 = string.Empty;
    string 歷史操作壓力變動範圍 = string.Empty;
    string 壓力計警報設定值 = string.Empty;
    string 流量計警報設定值 = string.Empty;
    string 前一年度警報發生頻率 = string.Empty;
    #endregion
    #region public
    ...
    //管線輸送接受資料
    public string _負責泵送或接收之控制室名稱 { set { 負責泵送或接收之控制室名稱 = value; } }
    public string _操作壓力 { set { 操作壓力 = value; } }
    public string _歷史操作壓力變動範圍 { set { 歷史操作壓力變動範圍 = value; } }
    public string _壓力計警報設定值 { set { 壓力計警報設定值 = value; } }
    public string _流量計警報設定值 { set { 流量計警報設定值 = value; } }
    public string _前一年度警報發生頻率 { set { 前一年度警報發生頻率 = value; } }
    #endregion
    ...
    public DataTable GetListStress()
    {
        SqlCommand oCmd = new SqlCommand();
        oCmd.Connection = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString"]);
        StringBuilder sb = new StringBuilder();

        sb.Append(@"select * from 天然氣_控制室_壓力計及流量計資料 where 資料狀態='A' and 業者guid=@業者guid ");
        if (!string.IsNullOrEmpty(年度))
            sb.Append(@" and 年度=@年度");
    ...
    public DataTable GetListPipe()
    {
        SqlCommand oCmd = new SqlCommand();
        oCmd.Connection = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString"]);
        StringBuilder sb = new StringBuilder();

        sb.Append(@"select * from 天然氣_控制室_管線輸送接收資料 where 資料狀態='A' and 業者guid=@業者guid ");
        if (!string.IsNullOrEmpty(年度))
            sb.Append(@" and 年度=@年度");
...
if(@yearCount > 0)
    ...
    public DataTable GetDataStress()
    {
        ...
        sb.Append(@"select * from 天然氣_控制室_壓力計及流量計資料 where guid=@guid and 資料狀態='A' ");
    ...
    public DataTable GetDataPipe()
    {
        SqlCommand oCmd = new SqlCommand();
        oCmd.Connection = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString"]);
        StringBuilder sb = new StringBuilder();

        sb.Append(@"select * from 天然氣_控制室_管線輸送接收資料 where guid=@guid and 資料狀態='A' ");
...
where guid=@guid and 資料狀態=@資料狀態 
 ");
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"insert into 天然氣_控制室_壓力計及流量計資料(  
年度,
業者guid,
管線識別碼,
自有端是否有設置壓力計,
...
@資料狀態  
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"update 天然氣_控制室_壓力計及流量計資料 set  
年度=@年度,
管線識別碼=@管線識別碼,
...
where guid=@guid and 資料狀態=@資料狀態 
 ");
    ...
    public void DeleteDataStress()
    {
        SqlCommand oCmd = new SqlCommand();
        oCmd.Connection = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString"]);
        oCmd.CommandText = @"update 天然氣_控制室_壓力計及流量計資料 set 
修改日期=@修改日期, 
修改者=@修改者, 
資料狀態='D' 
where guid=@guid ";
    ...
    public void InsertDataPipe(SqlConnection oConn, SqlTransaction oTran)
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"insert into 天然氣_控制室_管線輸送接收資料(  
年度,
業者guid,
管線識別碼,
負責泵送或接收之控制室名稱,
...
@資料狀態  
        ...
        oCmd.Parameters.AddWithValue("@業者guid", 業者guid);
        oCmd.Parameters.AddWithValue("@管線識別碼", 管線識別碼);
        oCmd.Parameters.AddWithValue("@負責泵送或接收之控制室名稱", 負責泵送或接收之控制室名稱);
        oCmd.Parameters.AddWithValue("@操作壓力", 操作壓力);
        oCmd.Parameters.AddWithValue("@歷史操作壓力變動範圍", 歷史操作壓力變動範圍);
        oCmd.Parameters.AddWithValue("@壓力計警報設定值", 壓力計警報設定值);
        oCmd.Parameters.AddWithValue("@流量計警報設定值", 流量計警報設定值);
        oCmd.Parameters.AddWithValue("@前一年度警報發生頻率", 前一年度警報發生頻率);
        oCmd.Parameters.AddWithValue("@修改者", 修改者);
        oCmd.Parameters.AddWithValue("@修改日期", DateTime.Now);
        oCmd.Parameters.AddWithValue("@建立者", 建立者);
        oCmd.Parameters.AddWithValue("@建立日期", DateTime.Now);
    ...
    public void UpdateDataPipe(SqlConnection oConn, SqlTransaction oTran)
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"update 天然氣_控制室_管線輸送接收資料 set  
年度=@年度,
管線識別碼=@管線識別碼,
負責泵送或接收之控制室名稱=@負責泵送或接收之控制室名稱,
操作壓力=@操作壓力,
歷史操作壓力變動範圍=@歷史操作壓力變動範圍,
壓力計警報設定值=@壓力計警報設定值,
流量計警報設定值=@流量計警報設定值,
前一年度警報發生頻率=@前一年度警報發生頻率,
修改者=@修改者, 
修改日期=@修改日期
where guid=@guid and 資料狀態=@資料狀態 
 ");
    ...
    public void DeleteDataPipe()
    {
        SqlCommand oCmd = new SqlCommand();
        oCmd.Connection = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString"]);
        oCmd.CommandText = @"update 天然氣_控制室_管線輸送接收資料 set 
修改日期=@修改日期, 
修改者=@修改者, 
資料狀態='D' 
where guid=@guid ";
