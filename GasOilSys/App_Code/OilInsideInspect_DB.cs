...
/// <summary>
/// OilInsideInspect_DB 的摘要描述
/// </summary>
public class OilInsideInspect_DB
{
...
select DISTINCT 年度 into #tmp from 石油_管線內部稽核表
...
if(@yearCount > 0)
    ...
    public DataTable GetData()
    {
        ...
        sb.Append(@"select * from 石油_管線內部稽核表 where 資料狀態='A' and guid=@guid ");
...
");
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"insert into 石油_管線內部稽核表( 
guid,
年度,
業者guid,
日期,
...
@資料狀態  
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"update 石油_管線內部稽核表 set  
年度=@年度,
日期=@日期,
執行單位=@執行單位,
稽核範圍=@稽核範圍,
