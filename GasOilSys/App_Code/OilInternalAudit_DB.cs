...
select DISTINCT 年度 into #tmp from 石油_儲槽內部稽核表
...
if(@yearCount > 0)
    ...
    public DataTable GetFileData()
    {
        ...
        sb.Append(@"select Btab.* from 石油_儲槽內部稽核表 Atab 
                    left join 附件檔 Btab on Atab.guid=Btab.guid 
                    where Btab.資料狀態='A' and Atab.guid=@guid ");
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"insert into 石油_儲槽內部稽核表( 
guid,
年度,
業者guid,
內部稽核日期,
...
@資料狀態  
        ...
        oCmd.Parameters.AddWithValue("@年度", 年度);
        oCmd.Parameters.AddWithValue("@業者guid", 業者guid);
        oCmd.Parameters.AddWithValue("@內部稽核日期", 內部稽核日期);
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"update 石油_儲槽內部稽核表 set  
年度=@年度,
內部稽核日期=@內部稽核日期,
執行單位=@執行單位,
稽核範圍=@稽核範圍,
...
");
    ...
    public void DelFile(SqlConnection oConn, SqlTransaction oTran)
    {
        ...
        sb.Append(@"
        update 石油_儲槽內部稽核表 
        set 佐證資料檔名='', 新檔名='', 佐證資料副檔名='', 佐證資料路徑='', 修改者=@修改者, 修改日期=@修改日期  
        where guid=@guid 
");
        SqlCommand oCmd = oConn.CreateCommand();
        oCmd.CommandText = sb.ToString();
