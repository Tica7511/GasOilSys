...
public class GasCIPSXY_DB
{
    ...
    public string _級距 { set { 級距 = value; } }
    ...
    public DataTable GetList()
    {
        ...
        sb.Append(@"select * from 天然氣_緊密電位檢測_xy座標 where 資料狀態='A' and 父層guid=@父層guid ");
...
select DISTINCT 年度 into #tmp from 天然氣_緊密電位檢測_xy座標
...
if(@yearCount > 0)
    ...
    public DataTable GetData()
    {
        ...
        sb.Append(@"select * from 天然氣_緊密電位檢測_xy座標 where guid=@guid and 資料狀態='A' ");
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"insert into 天然氣_緊密電位檢測_xy座標(  
父層guid,
年度,
業者guid,
x座標,
...
@資料狀態  
        ...
        oCmd.Parameters.AddWithValue("@級距", 級距);
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"update 天然氣_緊密電位檢測_xy座標 set  
x座標=@x座標,
y座標=@y座標,
級距=@級距,
修改者=@修改者, 
...
where guid=@guid and 資料狀態=@資料狀態 
 ");
    ...
    public void DeleteData()
    {
        SqlCommand oCmd = new SqlCommand();
        oCmd.Connection = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString"]);
        oCmd.CommandText = @"update 天然氣_緊密電位檢測_xy座標 set 
修改日期=@修改日期, 
修改者=@修改者, 
資料狀態='D' 
where guid=@guid ";
