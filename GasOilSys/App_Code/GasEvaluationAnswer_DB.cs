...
/// <summary>
/// EvaluationAnswer_DB 的摘要描述
/// </summary>
public class GasEvaluationAnswer_DB
{
...
");
    ...
    public DataTable GetCompanyAns()
	{
		...
		sb.Append(@"select 題目guid,答案, 檢視文件, 委員意見,填寫人員類別 from 天然氣_自評表答案 where 資料狀態='A' and 業者guid=@業者guid and 年度=@年度 ");
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"update 天然氣_自評表答案 
set 檢視文件=@檢視文件, 委員意見=@委員意見, 修改者=@修改者, 修改日期=@修改日期  
where 填寫人員類別=@填寫人員類別 and 業者guid=@業者guid and 
題目guid=@題目guid and 年度=@年度 and 資料狀態=@資料狀態 ");
        SqlCommand oCmd = oConn.CreateCommand();
        oCmd.CommandText = sb.ToString();
